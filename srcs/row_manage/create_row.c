...
void input_car(struct mg_http_message *request, struct mg_connection *conn)
{
	...
	{
		sprintf(response, "%s", "{\"status\":\"500\", \"message\": \"to create a new car, put name and price values\"}");
	...
	else
	{
		...
		{
			sprintf(response,  "%s", "{\"status\":\"500\", \"message\": \"error to register car in database\"}");
			mg_http_reply(conn, 500, NULL, response);
			status = 500;
		}
		else
